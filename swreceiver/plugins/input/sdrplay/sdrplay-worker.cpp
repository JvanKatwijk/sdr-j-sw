...
//	as a sanity check we could verify that dongleRate % outputRate == 0
	this	-> bandWidth	= float(bandWidth) / MHz (1);
...
//	Note: the "API check" has been done by the owner of this thread
	...
	err			= functions -> mir_sdr_SetDcMode (4, 1);
...
int32_t		next	= 0;
	...
	functions	-> mir_sdr_SetParam (102, 1);	// Dc correction
...
//	OK, data is now stored, now checking for updates
	   ...
	   if (_I_Buffer -> GetRingBufferReadAvailable () > outputRate / 10)
	      emit samplesAvailable (outputRate / 10);
	}
