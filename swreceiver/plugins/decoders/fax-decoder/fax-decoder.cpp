...
QWidget	*faxDecoder::createPluginWindow (int32_t rate, QSettings *s) {
	theRate			= rate;
	faxSettings		= s;
	...
	faxContainer	-> show ();
	localOscillator		= new Oscillator (rate);
	setup_faxDecoder	(faxSettings);
	fprintf (stderr, "faxDecoder is loaded\n");
	return theFrame;
}
...
bool	faxDecoder::initforRate	(int32_t rate) {
	if (theRate == rate)
	   return true;
...
int32_t	faxDecoder::rateOut	(void) {
	return theRate;
}

int16_t	faxDecoder::detectorOffset	(void) {
	return FAX_IF;
}
...
//
//	as always, we "doDecode" one sample at the time.
//
static
...
DSPFLOAT	temp		= (DSPFLOAT)samplenum / samplesperLine;
...
QString	file;
	...
	if (getName)
	   ...
	   file	= QFileDialog::getSaveFileName (theFrame,
	                                        tr ("save file as .."),
	                                        QDir::homePath (),
	                                        tr ("Images (*.jpg)"));
...
QT_BEGIN_NAMESPACE
Q_EXPORT_PLUGIN2(faxdecoder, faxDecoder)
QT_END_NAMESPACE
#endif
