...
public:
		...
		sdrplayLoader		(bool *);
		~sdrplayLoader		(void);
	pfn_mir_sdr_Init	mir_sdr_Init;
	pfn_mir_sdr_Uninit	mir_sdr_Uninit;
	pfn_mir_sdr_ReadPacket	mir_sdr_ReadPacket;
	...
	pfn_mir_sdr_ResetUpdateFlags
	                        xx_mir_sdr_ResetUpdateFlags;
	pfn_mir_sdr_SetParam	xx_mir_sdr_SetParam;
	void			mir_sdr_ResetUpdateFlags (int, int, int);
	void			mir_sdr_SetParam	(int, int);
private:
	HINSTANCE	Handle;
	bool		libraryLoaded;
};
