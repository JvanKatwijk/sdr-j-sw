...
//	Load the functions one by one
	mir_sdr_Init	= (pfn_mir_sdr_Init)
	                    GETPROCADDRESS (Handle, "mir_sdr_Init");
	if (mir_sdr_Init == NULL) {
	...
	mir_sdr_ReadPacket = (pfn_mir_sdr_ReadPacket)
	                    GETPROCADDRESS (Handle, "mir_sdr_ReadPacket");
	if (mir_sdr_ReadPacket == NULL) {
	...
	mir_sdr_SetParam	= (pfn_mir_sdr_SetParam)
	                    GETPROCADDRESS (Handle, "mir_sdr_SetParam");
	if (mir_sdr_SetParam == NULL) {
	   fprintf (stderr, "Could not find mir_sdr_SetParam\n");
	   return;
	}
