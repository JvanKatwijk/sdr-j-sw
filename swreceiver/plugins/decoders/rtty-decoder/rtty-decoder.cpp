...
QWidget	*rttyDecoder::createPluginWindow (int32_t rate, QSettings *s) {
	theRate		= rate;
	rttySettings	= s;

	myFrame		= new QFrame;
	setupUi (myFrame);
	fprintf (stderr, "rttyDecoder is loaded\n");
	setup_rttyDecoder	(rate);
	return myFrame;
}
//
//	The Dummy:
...
int32_t	rttyDecoder::rateOut		(void) {
	return theRate;
}

int16_t	rttyDecoder::detectorOffset	(void) {
	return rttyIF;
}

void	rttyDecoder::setup_rttyDecoder (int32_t WorkingR) {
QString	temp;
int16_t	k;
...
bool	rttyDecoder::initforRate	(int32_t workingRate) {
	if (workingRate == theRate)
	   return true;
	theRate = workingRate;
 ...
 */
void	rttyDecoder::doDecode (DSPCOMPLEX z) {
DSPCOMPLEX	local_copy;
double		FreqOffset;
int	res;
//	keep a local copy of the sample
	local_copy	= z;

	z	= z * LocalOscillator -> nextValue (rttyIF);
	z	= LPM_Filter -> Pass (z);
...
//	   rtty_showFreqCorrection ((float)rttyIF);
	...
	outputSample (real (local_copy), imag (local_copy));
}
/*
...
QT_BEGIN_NAMESPACE
Q_EXPORT_PLUGIN2(rttydecoder, rttyDecoder)
QT_END_NAMESPACE
#endif
